...
Epoll:SP_ReqData Epoll::fd2req[MAXFDS0];
...
void Epoll::acceptConnection(int listen_fd,int epoll_fd,const std::string path)
{
	...
	while((accept_fd = accept(listen_fd,(struct sockaddr*)&client_addr,&client_addr_len)) > 0)
	{
		...
		cout << ntoas(client_addr.sin_port) << endl;
		/*
		//TCP保活机制默认是关闭的
		int optval = 0;
		...
		*/
		...
		//设为非阻塞模式
		int ret = setsocketNoBlocking(accept_fd);
		if(ret < 0)
		{
		...
		//文件描述符可以读，边缘触发(ET)模式，保证一个socket连接在任一时刻只被一个线程处理
		__uint32_t _epo_event = EPOLLIN | EPOLLET | EPOLLONESHOT；
		Epol::epoll_add(accept_fd,req_info,_epo_event);
		//新增时间信息
...
//分发处理函数
std::vector<std::shared_ptr<RequestData>> Epoll::getEventRequest(int listen_fd,int events_num,const std::string path)
{
	...
	for(int i=0; i<events_num, ++i)
	{
		...
		else 
		{
			...
			if((events[i].events & EPOLLERR) || (events[i].events & EPOLLHUP))
			{
				printf("error event\n");
				if(fd2req[fd])
					fd2req[fd]->seperaterTimer();
				fd2req[fd].reset();
				continue;
			}
			//将请求任务加到线程池中
			//加入线程池之前Timer和request分离
			
			SP_ReqData cur_req = fd2reqq[fd];
